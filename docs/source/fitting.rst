...
----------------------------
The NLL is the most commonly used objective function for fitting. It is constructed using the ``nll`` method of a pdf node in the workspace:
...
Whatever the NLL options are, the NLL function can always be factorized into two parts: the *main term* and the *constraint term*. The main term depends on the regular observables and the constraint term depends on the global observables. The main term is the sum of the negative log likelihoods of each the entries in the dataset (accounting for any weights):
